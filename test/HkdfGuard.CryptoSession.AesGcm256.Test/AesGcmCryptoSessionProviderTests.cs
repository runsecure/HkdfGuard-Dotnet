...
public class AesGcmCryptoSessionProviderTests
{
    [Fact]
    public void Constructor_BuildsInitialSessionEagerly()
    {
        ...
        using var provider = new AesGcmCryptoSessionProvider(wrapper, "wrapped"u8.ToArray(), 60);
    ...
    public void Constructor_WhenKeyWrapperFails_Throws()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => new AesGcmCryptoSessionProvider(wrapper, "wrapped"u8.ToArray(), 60));
    }
    ...
    public void Constructor_WithExpirySecondsOutOfRange_ThrowsArgumentOutOfRangeException(int expirySeconds)
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() => new AesGcmCryptoSessionProvider(wrapper, "wrapped"u8.ToArray(), expirySeconds));
    }

    [Fact]
    public void GetSession_WithinExpiry_ReturnsTheSameCachedSession()
    {
        var wrapper = new FakeKeyWrapper();
        using var provider = new AesGcmCryptoSessionProvider(wrapper, "wrapped"u8.ToArray(), 60);

        var first = provider.GetSession();
        var second = provider.GetSession();
    ...
    [Fact]
    public void GetSession_AfterExpiry_RevealsFreshKeyAndDisposesThePreviousSession()
    {
        var wrapper = new FakeKeyWrapper();
        using var provider = new AesGcmCryptoSessionProvider(wrapper, "wrapped"u8.ToArray(), 1);
        var first = provider.GetSession();
        ...
        Assert.Throws<ObjectDisposedException>(() => first.Encrypt("hello"u8.ToArray(), new byte[33]));
    }
    ...
    {
        var wrapper = new FakeKeyWrapper();
        var provider = new AesGcmCryptoSessionProvider(wrapper, "wrapped"u8.ToArray(), 1);
